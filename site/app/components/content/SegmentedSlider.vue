...
import { buildSegmentedSliderModel } from './buildSegmentedSliderModel.ts';
...
<template>
  <div class="w-full space-y-2">
    ...
    <USlider
      :model-value="buildSegmentedSliderModel.indexFor(items, modelValue)"
      :min="0"
      :max="items.length - 1"
      :step="1"
      @update:model-value="emit('update:modelValue', buildSegmentedSliderModel.valueAt(items, Number($event), modelValue))"
    />
  </div>
</template>
