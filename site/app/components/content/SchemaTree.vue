...
const tree = computed(() => buildSchemaTree.build(
  framing.value,
  schemaName.value,
  roles.value,
  roleSortKeys.value,
  ...
  <UCard>
    ...
    <UTree :items="tree">
      <template #item-label="{ item }">
        <SchemaTreeLeafLabel
          :label="item.label"
          :is-leaf="buildSchemaTree.isLeaf(item)"
          :hex="buildSchemaTree.isLeaf(item) ? item.hex : undefined"
          :derived-from="buildSchemaTree.isLeaf(item) ? item.derivedFrom : undefined"
        />
      </template>
    </UTree>
    ...
    <FootnoteText>
      ...
      <DocAnchorLink href="#04-engine-api">
        Long-form Engine API
      </DocAnchorLink> for what every field does.
    </FootnoteText>
  </UCard>
</template>
