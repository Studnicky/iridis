...
<template>
  <div class="flex flex-col rounded-lg border border-default bg-elevated text-sm overflow-hidden break-inside-avoid mb-3">
    ...
    >
      ...
      <div
        v-else-if="role.isDerived"
        class="text-xs text-muted italic space-y-2"
      >
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
          <div
            v-if="role.def?.lightnessTarget !== undefined"
            class="bg-muted p-2 rounded border border-default/30 flex flex-col"
          >
          ...
          <div
            v-if="role.def?.lightnessClamp !== undefined"
            class="bg-muted p-2 rounded border border-default/30 flex flex-col"
          >
          ...
          <div
            v-if="role.def?.chromaTarget !== undefined"
            class="bg-muted p-2 rounded border border-default/30 flex flex-col"
          >
          ...
          <div
            v-if="role.def?.chromaClamp !== undefined"
            class="bg-muted p-2 rounded border border-default/30 flex flex-col"
          >
          ...
          <div
            v-if="role.def?.hueClamp !== undefined"
            class="bg-muted p-2 rounded border border-default/30 flex flex-col"
          >
        ...
        <div
          v-if="role.algorithmInfo"
          class="mt-2 pt-2 border-t border-default/30 space-y-1"
        >
          <div class="text-[10px] uppercase tracking-wide text-dimmed not-italic font-medium">
            Hue-derivation algorithm
          </div>
          <div class="not-italic text-highlighted">
            {{ ALGORITHM_LABELS[role.algorithmInfo.hueAlgorithm] ?? role.algorithmInfo.hueAlgorithm }}
          </div>
      ...
      <div
        v-else-if="role.isPinned"
        class="text-xs text-muted italic space-y-1"
      >
        <div>This role was explicitly pinned by the user, so distance-matching against candidate seeds was skipped entirely.</div>
        <div
          v-if="role.pinnedSeedHex"
          class="not-italic flex items-center gap-2 mt-1"
        >
          <div
            class="h-5 w-5 rounded shadow-inner flex-none border border-default"
            :style="{ backgroundColor: role.pinnedSeedHex }"
          />
        ...
        <div class="space-y-1">
          ...
          >
            ...
            <div class="flex-1 text-xs font-mono">
              {{ cand.hex.toLowerCase() }}
            </div>
            <div
              class="text-xs font-mono"
              :class="cand.isWinner ? 'text-primary font-bold' : 'text-muted'"
            >
              Δ {{ cand.dist.toFixed(4) }}
            </div>
          </div>
          <div
            v-if="role.candidates.length > 4"
            class="text-[10px] text-dimmed italic pl-1.5"
          >
      ...
      >
        ...
        <div class="flex items-center gap-3">
          <div
            class="h-6 w-6 rounded border border-default shadow-inner flex-none"
            :style="{ backgroundColor: role.clamp.seedHex }"
          />
        ...
        <div class="flex items-center gap-3">
          <div
            class="h-6 w-6 rounded border border-default shadow-inner flex-none"
            :style="{ backgroundColor: role.clamp.resolvedHex }"
          />
          <div class="flex flex-col min-w-0">
