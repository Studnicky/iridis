...
/**
 * Live Nuxt UI component surface — every control themed by the engine's --ui-*
 * tokens. One face of the cylinder.
 */
...
<template>
  <div class="space-y-5">
    ...
    <UInput placeholder="Search…" icon="i-material-symbols-search-rounded" />
    ...
    <UAlert
      ...
      description="Icon, border and text all read from engine-written --ui-* variables."
      icon="i-material-symbols-auto-awesome-rounded"
    />

    <div class="space-y-1">
      <div class="text-xs font-medium text-muted">Primary scale</div>
      <div class="flex gap-0.5">
        <div v-for="s in shades" :key="s" class="h-8 flex-1 rounded" :style="{ backgroundColor: `var(--ui-color-primary-${s})` }" :title="`primary-${s}`" />
      </div>
    </div>
  </div>
</template>
