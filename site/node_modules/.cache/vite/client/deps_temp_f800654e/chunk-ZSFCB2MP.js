...
var require_dataType = __commonJS({
  "../node_modules/ajv/dist/compile/validate/dataType.js"(exports) {
    ...
    var DataType2;
    (function(DataType3) {
      DataType3[DataType3["Correct"] = 0] = "Correct";
      DataType3[DataType3["Wrong"] = 1] = "Wrong";
    ...
    function coerceAndCheckDataType(it, types) {
      ...
      if (checkTypes) {
        const wrongType = checkDataTypes(types, data, opts.strictNumbers, DataType2.Wrong);
        gen.if(wrongType, () => {
          if (coerceTo.length)
    ...
    function checkDataType(dataType, data, strictNums, correct = DataType2.Correct) {
      const EQ = correct === DataType2.Correct ? codegen_1.operators.EQ : codegen_1.operators.NEQ;
      let cond;
      switch (dataType) {
        case "null":
      ...
      return correct === DataType2.Correct ? cond : (0, codegen_1.not)(cond);
...
// ../node_modules/@studnicky/json/dist/errors/JsonError.js
var JsonError = class extends BaseError {
  constructor(args) {
    super(args);
  }
};

// ../node_modules/@studnicky/json/dist/errors/PatchError.js
var PatchError = class extends JsonError {
  op;
  path;
  constructor(message, op, path) {
...
// ../node_modules/@studnicky/json/dist/json/Clone.js
var Clone = class {
  // ---------------------------------------------------------------------------
  ...
  static cloneArray(value) {
    const result = value.map((item) => {
      const result2 = this.cloneValue(item);
      return result2;
    });
    return result;
  }
...
// ../node_modules/@studnicky/json/dist/json/DataType.js
var DataType = class {
  // ---------------------------------------------------------------------------
  ...
  static walkForCycle(value, seen) {
    if (value === null || typeof value !== "object") {
      return false;
    }
    if (seen.has(value)) {
  ...
  static compareObjects(left, right) {
   ...
   */
  static hasCycle(value) {
    const result = this.walkForCycle(value, /* @__PURE__ */ new WeakSet());
    return result;
  }
...
// ../node_modules/@studnicky/json/dist/json/Frozen.js
var Frozen = class {
  // ---------------------------------------------------------------------------
  ...
  static deepFreeze(value) {
    const result = this.freezeValue(value, /* @__PURE__ */ new WeakSet());
    return result;
  }
};

// ../node_modules/@studnicky/json/dist/json/Hash.js
var FNV_OFFSET_BASIS = 2166136261;
var FNV_PRIME = 16777619;
var UINT32_MASK = 4294967295;
var Hash = class {
  // ---------------------------------------------------------------------------
  ...
  static hashValue(value) {
    ...
    if (DataType.isRecord(value)) {
      ...
      const parts = keys.map((k) => {
        const result = `${k}:${this.hashValue(value[k])}`;
        return result;
      });
      return `{${parts.join(",")}}`;
    }
...
// ../node_modules/@studnicky/json/dist/json/Merge.js
var Merge = class {
  // ---------------------------------------------------------------------------
   ...
   */
  static mergeValues(baseValue, overlayValue) {
  ...
  // Private helpers
  // ---------------------------------------------------------------------------
  static assertDeepMergeResult(value) {
    void value;
  }
  // ---------------------------------------------------------------------------
  // Public static API
   ...
   */
  static deep(baseValue, overlayValue) {
    const result = this.mergeValues(baseValue, overlayValue);
    this.assertDeepMergeResult(result);
    return result;
  }
};

// ../node_modules/@studnicky/json/dist/json/Patch.js
var VALID_OPS = /* @__PURE__ */ new Set(["add", "copy", "move", "remove", "replace", "test"]);
var Patch = class _Patch {
  ...
  constructor(operations = []) {
    ...
    for (let i = 0; i < opsLen; i += 1) {
      ...
      if (!VALID_OPS.has(operation.op)) {
        throw new PatchError(`Invalid patch operation "${String(operation.op)}"; must be one of: ${[...VALID_OPS].join(", ")}`, String(operation.op), operation.path);
      }
    }
    this.operations = ops;
  }
  ...
  toString() {
    const result = this.operations.map((op) => {
      const result2 = this.describeOp(op);
      return result2;
    }).join(", ");
    return result;
  }
  ...
  parsePath(path) {
    ...
    return path.slice(1).split("/").map((part) => {
      const result = part.replace(/~1/gu, "/").replace(/~0/gu, "~");
      return result;
    });
  }
  ...
  applyReplace(target, op) {
    if (!this.hasValue(target, op.path)) {
      throw new PatchError(`Cannot replace non-existent path: ${op.path}`, op.op, op.path);
    }
    this.setValue(target, op.path, op.value);
  }
  /** Apply a single RFC-6902 `remove` operation. */
  applyRemove(target, op) {
  ...
  applyTest(target, op) {
    ...
    if (actual !== op.value) {
      throw new PatchError(`Test failed at ${op.path}: expected ${String(op.value)}, got ${String(actual)}`, op.op, op.path);
    }
  }
  static operationAppliers = {
    "add": (self, target, op) => {
      self.applyAdd(target, op);
    },
    "copy": (self, target, op) => {
      self.applyCopy(target, op);
    },
    "move": (self, target, op) => {
      self.applyMove(target, op);
    },
    "remove": (self, target, op) => {
      self.applyRemove(target, op);
    },
    "replace": (self, target, op) => {
      self.applyReplace(target, op);
    },
    "test": (self, target, op) => {
      self.applyTest(target, op);
    }
  };
  /** Apply a single RFC-6902 operation to `target`. */
  applyOperation(target, op) {
    const applier = _Patch.operationAppliers[op.op];
    if (applier === void 0) {
...
// ../node_modules/@studnicky/json/dist/json/Path.js
var DANGEROUS_PROPERTIES = /* @__PURE__ */ new Set([
  "__defineGetter__",
  "__defineSetter__",
  "__lookupGetter__",
  "__lookupSetter__",
...
var VALID_IDENTIFIER = /^[$_a-zA-Z][$_a-zA-Z0-9]*$/u;
var Path = class {
  // ---------------------------------------------------------------------------
   ...
   */
  static toAccess(jsonPointer) {
    if (jsonPointer === "" || jsonPointer === "/") {
      return "";
    }
    const segments = jsonPointer.split("/").slice(1).map((seg) => {
      const result2 = seg.replaceAll("~1", "/").replaceAll("~0", "~");
      return result2;
    });
    let result = "";
  ...
  static get(obj, path, options) {
    ...
    if (options?.maxDepth !== void 0 && parts.length > options.maxDepth) {
      return void 0;
    }
    let current = obj;
    const partsLen = parts.length;
    for (let i = 0; i < partsLen; i++) {
      ...
      if (part.includes("[") && part.includes("]")) {
        ...
        const arrayValue = current[fieldName];
...
// ../node_modules/@studnicky/json/dist/json/Sort.js
var Sort = class {
  // ---------------------------------------------------------------------------
  // Public static API
  // ---------------------------------------------------------------------------
   ...
   */
  static natural(a, b) {
    const result = a.localeCompare(b, void 0, { "numeric": true, "sensitivity": "base" });
    return result;
  }
  /**
...
// ../node_modules/@studnicky/json/dist/json/StructuralHash.js
var STRUCTURAL_METADATA_KEYS = /* @__PURE__ */ new Set(["$id", "description", "title"]);
var StructuralHash = class {
  // ---------------------------------------------------------------------------
   ...
   */
  static stripMetadata(value) {
    if (Array.isArray(value)) {
      return value.map((item) => {
        const result2 = this.stripMetadata(item);
        return result2;
      });
    }
...
// ../node_modules/@studnicky/json/dist/schema/AjvInstance.js
var import_ajv_formats = __toESM(require_dist(), 1);
var import__ = __toESM(require__(), 1);
var AjvClass = import__.default.default ?? import__.default;
var addFormatsFunction = import_ajv_formats.default.default ?? import_ajv_formats.default;
var ajvInstance = new AjvClass({
  "allErrors": true,
...
// ../node_modules/@studnicky/json/dist/schema/SchemaValidator.js
var SchemaValidator = class _SchemaValidator {
  ...
  static formatErrors(errors) {
    ...
    return errors.map(_SchemaValidator.formatError).join("; ");
  }
...
//# sourceMappingURL=chunk-ZSFCB2MP.js.map
