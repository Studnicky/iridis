 ...
 */
import { MAX_DISPATCHER_CONNECTIONS, MAX_PIPELINING } from '../../constants/index.js';
...
class DispatcherValidator {
     ...
     */
    static nonNegativeNumber(value, path) {
     ...
     */
    static optionalNonNegativeNumber(value, path) {
     ...
     */
    static positiveIntegerWithMax(value, path, max) {
        if (typeof value !== 'number') {
            throw new ConfigurationError(`${path} must be a number`);
        }
        if (value < 1) {
     ...
     */
    static positiveInteger(value, path) {
        if (typeof value !== 'number') {
            throw new ConfigurationError(`${path} must be a number`);
        }
        if (value < 1) {
     ...
     */
    static optionalPositiveInteger(value, path) {
        if (value === undefined || value === null) {
            return;
        }
     ...
     */
    static optionalNonEmptyString(value, path) {
        if (value === undefined || value === null) {
            return;
        }
     ...
     */
    static maxResponseSize(value, path) {
        if (typeof value !== 'number') {
            throw new ConfigurationError(`${path} must be a number`);
        }
        if (value !== -1 && value < 0) {
    ...
    static validate(val) {
        ...
        for (const [key, value] of Object.entries(dispatcher)) {
...
const VALIDATORS = {
    ...
    'keepAliveTimeoutThreshold': (value) => {
    ...
    'strictContentLength': (value) => {
        const result = DispatcherValidator.boolean(value, 'dispatcher.strictContentLength');
        return result;
    }
};
export const validateDispatcher = DispatcherValidator.validate;
//# sourceMappingURL=validateDispatcher.js.map