 ...
 */
import { validateAutoGenerateRequestId, validateDispatcher, validateHeaders, validateMetadata, validateName, validateOptions, validateParams, validateRequestIdGenerator, validateTimeout, validateURL } from '../config/schemas/index.js';
import { AbortError, BodyTimeoutError, ConfigurationError, ConnectTimeoutError, HeadersTimeoutError, SocketError, SocketExhaustionError, TimeoutError } from '../errors/index.js';
...
export class FetchClient {
     ...
     */
    createRequestMetadata(path, method, options) {
     ...
     */
    async executeRequest(requestContext, method, requestId) {
     ...
     */
    handleSocketExhaustion(url, errorCode, method, requestId, duration) {
        if (this.dispatcher === undefined) {
            return undefined;
        }
        ...
        const stats = this.dispatcher.getAgent().stats[origin];
        this.onRequestError(new Error(`Connection pool exhaustion: ${errorCode}`), method, requestId, url, duration);
    ...
    onRequestStart(_method, _path, _requestId, _url) { }
    /** Fires when an HTTP request fails. */
    onRequestError(_error, _method, _requestId, _url, _durationMs) { }
    /** Fires when an HTTP response is successfully received. */
    onResponseSuccess(_method, _requestId, _statusCode, _durationMs) { }
    /** Fires when an HTTP response with non-2xx status is received. */
    onResponseError(_method, _requestId, _statusCode, _durationMs) { }
    /** Fires when a request is aborted by a timeout. */
    onTimeout(_method, _requestId, _url, _timeoutMs) { }
    /** Fires when a request is aborted by the caller. */
    onAbort(_method, _requestId, _url) { }
    ...
    mergeOptions(options) {
        ...
        const optionsDispatcher = options.dispatcher;
        const configOptionsDispatcher = this.config.options?.dispatcher;
     ...
     */
    prepareBodyRequest(method, options) {
        const { body, ...restOptions } = options ?? {};
        const serializedBody = this.serializeBody(body);
        const fetchOptions = { ...restOptions, 'method': method };
     ...
     */
    wrapUndiciError(error, url, method, requestId, duration) {
