 ...
 */
import { AbortError, ConfigurationError, TimeoutError } from '../errors/index.js';
/**
...
export class HttpMethods {
     ...
     */
    static toRequestInit(opts, signal) {
        // Destructure to exclude custom properties (dispatcher, json, metadata, requestId, timeout)
        const { 'dispatcher': _d, 'json': _j, 'metadata': _m, 'requestId': _r, 'signal': _s, 'timeout': _t, ...standardProps } = opts;
        if (signal === undefined) {
    ...
    static async fetch(url, opts = {}) {
        ...
        const { 'signal': externalSignal, timeout, ...fetchOptions } = opts;
     ...
     */
    static async fetchWithoutTimeout(url, fetchOptions, externalSignal) {
        try {
     ...
     */
    static async fetchWithTimeout(url, fetchOptions, externalSignal, timeout) {
     ...
     */
    static async patch(url, opts) {
        const { body, ...restOpts } = opts ?? {};
        const fetchOptions = { ...restOpts, 'method': 'PATCH' };
     ...
     * @param opts - Fetch options including optional body (auto-serialized to JSON if object/array; raw string/Buffer sent as-is)
     */
    static async post(url, opts) {
        const { body, ...restOpts } = opts ?? {};
        const fetchOptions = { ...restOpts, 'method': 'POST' };
        const serialized = HttpMethods.serializeRequestBody(body);
        if (serialized !== undefined) {
            fetchOptions.body = serialized;
            if (HttpMethods.needsContentType(body)) {
     ...
     * @param opts - Fetch options including optional body (auto-serialized to JSON if object/array; raw string/Buffer sent as-is)
     */
    static async put(url, opts) {
        const { body, ...restOpts } = opts ?? {};
        const fetchOptions = { ...restOpts, 'method': 'PUT' };
        const serialized = HttpMethods.serializeRequestBody(body);
        if (serialized !== undefined) {
            fetchOptions.body = serialized;
            if (HttpMethods.needsContentType(body)) {
