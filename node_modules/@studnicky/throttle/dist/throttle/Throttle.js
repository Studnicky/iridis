...
import { ADAPTIVE_CONFIG_KEYS, DEFAULT_ADAPTIVE_CONFIG, DEFAULT_BUFFER_CAPACITY, DEFAULT_THROTTLE_CONCURRENCY, DEFAULT_TIMEOUT, EMPTY_LENGTH, FIRST_ARRAY_INDEX, INITIAL_COUNTER, MIN_ADJUSTMENT_INTERVAL, MIN_CONCURRENCY_LIMIT, MIN_SAMPLE_WINDOW, NO_DELAY_MS, PERCENTILE_P50, PERCENTILE_P95, PERCENTILE_P99, THROTTLE_CONFIG_KEYS } from '../constants/index.js';
import { ConfigurationError, ThrottleAbortedError, ThrottleDrainingError } from '../errors/index.js';
...
export class Throttle {
     ...
     */
    #state = 'idle';
    activeCount = INITIAL_COUNTER;
    activeOperations = new Set();
    adjustmentCount = INITIAL_COUNTER;
    completionPromise = null;
    config;
    lastAdjustmentTime = INITIAL_COUNTER;
    latencyBuffer;
    observers = [];
    queue;
    totalExecuted = INITIAL_COUNTER;
    /**
    ...
    guard(from, to) {
        ...
        if (from === 'idle' && to === 'draining') {
            return true;
        }
        if (from === 'active' && to === 'draining') {
            return true;
        }
        if (from === 'draining' && to === 'idle') {
            return true;
        }
        return false;
    }
     ...
     */
    calculateNewLimit(adaptive, p95) {
    ...
    async execute(fn) {
        ...
        return await new Promise((resolveExecute, rejectExecute) => {
            this.acquireSlot().then(() => {
                ...
                const operation = {
                    'completed': false,
                    'resolve': () => { resolveExecute(undefined); }
                };
                this.activeOperations.add(operation);
                const operationStartTime = Date.now();
                fn().then((result) => { this.handleOperationSuccess(operation, result, operationStartTime, resolveExecute); }, (error) => { this.handleOperationError(operation, error, rejectExecute); });
            })
                .catch(rejectExecute);
        });
    }
     ...
     */
    async handleGracePeriod(timeout) {
     ...
     */
    handleOperationError(operation, error, rejectExecute) {
        if (operation.completed) {
            return;
        }
        operation.completed = true;
     ...
     */
    handleOperationSuccess(operation, result, operationStartTime, resolveExecute) {
        if (operation.completed) {
            return;
        }
        operation.completed = true;
     ...
     */
    onAcquire(_activeCount, _queuedCount) { }
    /**
     ...
     */
    onContended(_activeCount, _queuedCount) { }
    /**
     ...
     */
    onWindowSlide(_activeCount, _queuedCount) { }
    /**
     * Fires when abort is executed.
     */
    onAbortStart(_cancelledCount) { }
    /**
     * Fires when adaptive concurrency adjusts the limit.
     */
    onAdaptiveAdjust(_previousLimit, _newLimit) { }
    /**
     * Fires when drain is initiated.
     */
    onDrainStart(_activeCount, _queuedCount) { }
    /**
     ...
     */
    onDrainComplete(_totalExecuted) { }
    /**
     ...
     */
    onRelease(_activeCount, _totalExecuted) { }
    /**
     ...
     */
    scaleConcurrency(adaptive, _p95, direction) {
     ...
     */
    shouldAdjustConcurrency(adaptive) {
     ...
     */
    async waitForGracePeriod(timeout) {
        if (this.isComplete()) {
            return false;
        }
    ...
    static ADAPTIVE_DEFAULTS = {
        ...DEFAULT_ADAPTIVE_CONFIG,
        'targetLatencyMs': NO_DELAY_MS
    };
    static validateAdaptiveFields(adaptiveObj) {
    ...
    static buildEnabledAdaptiveConfig(adaptiveObj) {
        const d = Throttle.ADAPTIVE_DEFAULTS;
        const minConcurrency = adaptiveObj.minConcurrency ?? d.minConcurrency;
        const maxConcurrency = adaptiveObj.maxConcurrency ?? d.maxConcurrency;
        const scaleUpThreshold = adaptiveObj.scaleUpThreshold ?? d.scaleUpThreshold;
        const scaleDownThreshold = adaptiveObj.scaleDownThreshold ?? d.scaleDownThreshold;
        ...
        return {
            'adjustmentInterval': adaptiveObj.adjustmentInterval ?? d.adjustmentInterval,
            'enabled': true,
            'maxConcurrency': maxConcurrency,
            'minConcurrency': minConcurrency,
            'sampleWindow': adaptiveObj.sampleWindow ?? d.sampleWindow,
            'scaleDownThreshold': scaleDownThreshold,
            'scaleUpThreshold': scaleUpThreshold,
            'stepSize': adaptiveObj.stepSize ?? d.stepSize,
            'targetLatencyMs': adaptiveObj.targetLatencyMs
        };
    }
    static validateAdaptiveConfig(adaptive) {
    ...
    static validateConfig(config) {
        ...
        const result = { 'concurrencyLimit': concurrencyLimit };
        if (adaptive !== undefined) {
