...
class AstHelpers {
    static getEntityBaseName(filename) {
...
const isConstTypeAnnotation = (typeAnnotation) => {
    if (!isJsonObject(typeAnnotation)) {
        return false;
    }
...
const isSchemaAsConst = (declarator) => {
    if (!isJsonObject(declarator)) {
        return false;
    }
    ...
    if (initType === 'TSSatisfiesExpression') {
        ...
        if (!isJsonObject(expression) || AstHelpers.getNodeType(expression) !== 'TSAsExpression') {
            return false;
        }
...
const isFromSchemaRef = (typeAnnotation) => {
    if (!isJsonObject(typeAnnotation) || AstHelpers.getNodeType(typeAnnotation) !== 'TSTypeReference') {
        return false;
    }
    const { typeName } = typeAnnotation;
    if (!isJsonObject(typeName)) {
        return false;
    }
    // Handle both plain Identifier and qualified name (e.g. Ns.FromSchema)
    const fromSchemaName = typeName.name === 'FromSchema' ||
    ...
    const arg = params[0];
    if (!isJsonObject(arg) || AstHelpers.getNodeType(arg) !== 'TSTypeQuery') {
        return false;
    }
    const { exprName } = arg;
    if (!isJsonObject(exprName)) {
        return false;
    }
    return (exprName).name === 'Schema';
};
const isTypeFromSchema = (decl) => {
    if (!isJsonObject(decl)) {
        return false;
    }
    ...
    if (isFromSchemaRef(typeAnnotation)) {
        return true;
    }
...
const isSchemaValidatorCompile = (init) => {
    if (!isJsonObject(init) || AstHelpers.getNodeType(init) !== 'CallExpression') {
        return false;
    }
    const { callee } = init;
    if (!isJsonObject(callee) || AstHelpers.getNodeType(callee) !== 'MemberExpression') {
        return false;
    }
    const { object, property } = callee;
    if (!isJsonObject(object) || (object).name !== 'SchemaValidator') {
        return false;
    }
    if (!isJsonObject(property) || (property).name !== 'compile') {
        return false;
    }
    // Require an explicit `<Type>` argument so the guard narrows to the entity Type.
    let typeParams = init.typeArguments;
    if (!isJsonObject(typeParams)) {
...
const isValidateTypeGuard = (decl) => {
    if (!isJsonObject(decl)) {
        return false;
    }
    ...
    if (declType === 'VariableDeclaration') {
        ...
        if (Array.isArray(declarations) && declarations.length > 0 && isJsonObject(declarations[0])) {
            ...
            if (isSchemaValidatorCompile(firstDeclarator.init)) {
                return true;
            }
        }
    }
    ...
    if (!isJsonObject(predicateNode) || AstHelpers.getNodeType(predicateNode) !== 'TSTypePredicate') {
        return false;
    }
    ...
    if (!isJsonObject(predTypeAnnotation)) {
        return false;
    }
    // May be wrapped in TSTypeAnnotation
    let refNode = predTypeAnnotation;
    if (AstHelpers.getNodeType(refNode) === 'TSTypeAnnotation') {
        refNode = refNode.typeAnnotation;
    }
    if (!isJsonObject(refNode) || AstHelpers.getNodeType(refNode) !== 'TSTypeReference') {
        return false;
    }
...
const scanNamespaceBody = (bodyNode) => {
    const result = {
        'hasSchema': false,
    ...
    for (let i = 0; i < bodyLen; i += 1) {
        ...
        if (declType === 'VariableDeclaration') {
            ...
            for (let di = 0; di < declsLen; di += 1) {
                ...
                if (!isJsonObject(d) || !isJsonObject(d.id)) {
                    continue;
                }
                const { name } = d.id;
                if (name === 'Schema') {
...
export const entityNamespace = {
    'create': (context) => {
        ...
        if (filename === '<input>' || !isEntityFile(filename)) {
            return {};
        }
        ...
        return {
            'Program:exit': (program) => {
                ...
                for (let ei = 0; ei < exportsLen; ei += 1) {
                    ...
                    const reportNode = exportStmt;
                    if (!members.hasSchema) {
                        context.report({ 'messageId': 'missingSchema', 'node': reportNode });
                    }
                    else if (!members.hasSchemaAsConst) {
                        context.report({ 'messageId': 'schemaNotConst', 'node': reportNode });
                    }
                    if (!members.hasType) {
                        context.report({ 'messageId': 'missingType', 'node': reportNode });
                    }
                    else if (!members.hasTypeFromSchema) {
                        context.report({ 'messageId': 'typeNotFromSchema', 'node': reportNode });
                    }
                    if (!members.hasValidate) {
                        context.report({ 'messageId': 'missingValidate', 'node': reportNode });
                    }
                    else if (!members.hasValidateTypeGuard) {
                        context.report({ 'messageId': 'validateNotTypeGuard', 'node': reportNode });
                    }
                }
            }
        };
...
//# sourceMappingURL=entityNamespace.js.map