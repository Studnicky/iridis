import { IndexKind, SymbolFlags, TypeFlags } from 'typescript';
/**
...
class ContextHelpers {
    static getServices(context) {
        const result = context.sourceCode.parserServices;
        return result;
    }
}
const isSerializable = (type, checker, visited) => {
    if (visited.has(type)) {
        return true;
    }
    visited.add(type);
    ...
    if (type.isUnion()) {
        ...
        for (let i = 0; i < unionLength; i++) {
            if (!isSerializable(unionTypes[i], checker, visited)) {
                return false;
            }
        }
        return true;
    ...
    if (type.isIntersection()) {
        ...
        for (let i = 0; i < intersectionLength; i++) {
            if (!isSerializable(intersectionTypes[i], checker, visited)) {
                return false;
            }
        }
        return true;
    ...
    if (checker.isArrayType(type)) {
        ...
        for (let i = 0; i < typeArgsLength; i++) {
            if (!isSerializable(typeArgs[i], checker, visited)) {
                return false;
            }
        }
        return true;
    ...
    if ((flags & TypeFlags.Object) !== 0) {
        ...
        for (let i = 0; i < propsLength; i++) {
            ...
            if (!isSerializable(propType, checker, visited)) {
                return false;
            }
        }
...
export const interfaceMustBeContract = {
    'create': (context) => {
        ...
        const isMerged = (node, name) => {
            ...
            for (let index = 0; index < bodyLength; index++) {
                const bodyNode = programBody[index];
                if (!isObject(bodyNode)) {
                    continue;
                }
        ...
        const fixInterface = (node, rawNode) => {
            return (fixer) => {
                const fixes = [];
        ...
        return {
            'TSInterfaceDeclaration': (node) => {
                ...
                if (interfaceType.getCallSignatures().length > 0) {
                    return;
                }
                if (interfaceType.getConstructSignatures().length > 0) {
                    return;
                }
                ...
                for (let i = 0; i < propsLength; i++) {
                    ...
                    const propType = checker.getTypeOfSymbolAtLocation(prop, decl);
                    if (!isSerializable(propType, checker, new Set())) {
                        return;
                    }
                }
                // Check index types
                const stringIndexType = checker.getIndexTypeOfType(interfaceType, IndexKind.String);
                if (stringIndexType !== undefined && !isSerializable(stringIndexType, checker, new Set())) {
                    return;
                }
                const numberIndexType = checker.getIndexTypeOfType(interfaceType, IndexKind.Number);
                if (numberIndexType !== undefined && !isSerializable(numberIndexType, checker, new Set())) {
                    return;
                }
    ...
    'meta': {
        'docs': {
            'description': 'Interfaces express runtime contracts (functions, constructors, class references). A pure JSON-serializable data shape must be a schema-derived `type XxxType`, not an `interface`.'
        },
        'fixable': 'code',
        'messages': {
            'dataShapeMustBeType': "Interface '{{name}}' resolves to a fully JSON-serializable data shape with no contract signal (no call/construct signature, and no member typed as a function, constructor, or class instance). Data shapes must be declared as a schema-derived `type {{name}}Type` in an entity; `interface` is reserved for runtime contracts."
        },
        'schema': [
            {
...
//# sourceMappingURL=interfaceMustBeContract.js.map