const DEFAULT_VERB_PREFIXES = new Set([
...
const isVerbNounName = (name, verbPrefixes) => {
...
class PrefixHelpers {
    ...
    static extractVerbNoun(name, verbPrefixes) {
        const prefixes = [...verbPrefixes];
        const prefixesLen = prefixes.length;
        for (let i = 0; i < prefixesLen; i += 1) {
            const prefix = prefixes[i];
...
export const noFreestandingVerbNoun = {
    'create': (context) => {
        ...
        const onFunctionDeclaration = (node) => {
            ...
            if (!isVerbNounName(name, prefixSet)) {
                return;
            }
            report(node, name);
        };
        const onVariableDeclaration = (node) => {
            if (!isModuleScope(node)) {
                return;
            }
...
//# sourceMappingURL=noFreestandingVerbNoun.js.map