...
- Inline color pickers on every `<IridisDemo>`: each example now shows its own seed-color row with native `<input type="color">` per seed, an `× remove` per row, and a `+ add color` button (constrained to the schema's 1–8). The demo's local seeds default to the global config and stay linked until the user touches them; once touched, a "reset to global" link appears. Reactivity is bidirectional: sidebar-config changes flow into demos that haven't been touched, and demo-level changes don't disturb the global config.
