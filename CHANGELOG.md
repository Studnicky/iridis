...
- **Absolute `hue` on role definitions.** A role may declare `hue` (OKLCH degrees, [0, 360)) as an absolute target that takes precedence over the relative `hueOffset` in every resolution path — `resolve:roles` for directly-assigned roles and `expand:family` for `derivedFrom` roles. This lets a role inherit its source's lightness/chroma character while pinning a fixed hue (e.g. `success` stays green regardless of the brand seed). Previously `hueOffset` meant "absolute" in `resolve:roles` but "relative to the source" in `expand:family`; `hue` removes that ambiguity.
