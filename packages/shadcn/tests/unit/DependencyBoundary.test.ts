...
const SHADCN_DIRECTORY = join(ROOT_DIRECTORY, 'packages/shadcn');
...
class ShadcnTestDependencyBoundary {
  private static sourceFiles(directory: string): string[] {
    const files: string[] = [];
    for (const entry of readdirSync(directory, { 'withFileTypes': true })) {
      const path = join(directory, entry.name);
      if (entry.isDirectory()) {
        files.push(...ShadcnTestDependencyBoundary.sourceFiles(path));
  ...
  public static undeclaredImports(): string[] {
    ...
    const packageManifest = PackageManifest.parse(join(SHADCN_DIRECTORY, 'package.json'));
    ...
    const testDirectory = join(SHADCN_DIRECTORY, 'tests');
    for (const path of ShadcnTestDependencyBoundary.sourceFiles(testDirectory)) {
      const imports = ts.preProcessFile(readFileSync(path, 'utf8'), true, true).importedFiles;
      for (const imported of imports) {
        const packageName = ShadcnTestDependencyBoundary.packageName(imported.fileName);
        if (packageName !== undefined && !declared.has(packageName)) {
          violations.add(`${relative(SHADCN_DIRECTORY, path)} imports ${imported.fileName}`);
        }
      }
    }
...
await test('shadcn tests import only declared package dependencies', () => {
  assert.deepEqual(ShadcnTestDependencyBoundary.undeclaredImports(), []);
});
