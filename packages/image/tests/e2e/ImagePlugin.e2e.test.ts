...
import type {
  GalleryHarmonizeDetailsInterfaceType
...
abstract class CandidateSummary {
...
class ImageTestFixture {
  static candidateSummaries(value: JsonValueType): CandidateSummary[] | undefined {
    ...
    for (const candidate of value) {
      ...
      summaries.push({
        ...
        'colorCount': candidate.colors.length,
        'k': candidate.k,
  ...
  static harmonizeDetails(value: JsonValueType): GalleryHarmonizeDetailsInterfaceType | undefined {
    if (value === null || Array.isArray(value) || typeof value !== 'object') {return undefined;}
    if (
      typeof value.after !== 'string'
      || typeof value.before !== 'string'
      || typeof value.deltaE !== 'number'
  ...
  static imageData(
    ...
    return { 'data': data, 'height': 1, 'width': pixels.length };
  }

  static metadata(state: PaletteStateInterface, key: string): JsonValueType {
    if (key.length === 0) {return null;}
    return JsonValue.from(state.metadata[key]);
  }
}
...
const pluginShapeScenarios: readonly ScenarioInterface<PluginShapeInput, PluginShapeOutput>[] = [
  ...
  },
  {
    'assert': function(_output, error) {
      ...
      const names = imagePlugin.tasks().map((t) => { const result = t.name; return result; }).sort();
...
await new ScenarioRunner<PluginShapeInput, PluginShapeOutput>(
  ...
  (input) => {return { 'detail': input.label, 'ok': true };}
...
abstract class ExtractInput {
  abstract readonly 'input': InputInterface;
}
abstract class ExtractOutput {
  abstract readonly 'algorithmMeta': string | undefined;
  abstract readonly 'colorCount': number;
  abstract readonly 'hasDominantMeta': boolean;
...
const extractScenarios: readonly ScenarioInterface<ExtractInput, ExtractOutput>[] = [
  {
    'assert': function(output, error) {
      ...
      assert.strictEqual(output!.hasDominantMeta, true, '[cell=2, scenario=median-cut-hex] dominantColors is array');
    },
    'input': {
      'input': {
        'bypass':    undefined,
  ...
  },
  {
    ...
    'input': {
      'input': {
        ...
        'emit':      undefined,
        'maxColors': undefined,
        'metadata':  { 'gallery': { 'algorithm': 'median-cut', 'k': 3 } },
        'roles':     undefined,
        'runtime':   undefined
      }
    },
  ...
  },
  {
    ...
    'input': {
      'input': {
        'bypass':    undefined,
        'colors': [
          ImageTestFixture.imageData([
            ...(Array<null>(80).fill(null).map<[number, number, number]>(() => {return [200, 10, 10];})),
            ...(Array<null>(80).fill(null).map<[number, number, number]>(() => {return [10, 200, 10];})),
            ...(Array<null>(80).fill(null).map<[number, number, number]>(() => {return [10, 10, 200];})),
            ...(Array<null>(80).fill(null).map<[number, number, number]>(() => {return [200, 200, 10];}))
          ])
        ],
        'contrast':  undefined,
        'emit':      undefined,
        'maxColors': undefined,
        'metadata': { 'gallery': { 'algorithm': 'delta-e', 'k': 3 } },
        'roles':     undefined,
        'runtime':   undefined
      }
    },
...
await new ScenarioRunner<ExtractInput, ExtractOutput>(
  'ImagePlugin :: cell-2 :: gallery:extract',
  (input) => {
    const engine = ImageTestFixture.engine();
    engine.pipeline(['intake:imagePixels', 'intake:hex', 'gallery:histogram', 'gallery:extract']);
    const state = engine.run(input.input);
    const dominantMetadata = ImageTestFixture.metadata(state, 'gallery:dominantColors');
    const galleryConfig = ImageTestFixture.metadata(state, 'gallery');
    const algorithmMeta = galleryConfig !== null && !Array.isArray(galleryConfig) && typeof galleryConfig === 'object' && typeof galleryConfig.algorithm === 'string'
      ? galleryConfig.algorithm
      : undefined;
    return {
      'algorithmMeta': algorithmMeta,
      'colorCount':    state.colors.length,
      'hasDominantMeta': Array.isArray(dominantMetadata),
      'state': state
    };
  }
...
abstract class ExtractCandidatesInput {
  abstract readonly 'input': InputInterface;
  abstract readonly 'withExtract'?: boolean;
}
abstract class ExtractCandidatesOutput {
  abstract readonly 'candidates': CandidateSummary[] | undefined;
  abstract readonly 'colorCountAfter': number;
  abstract readonly 'hasDominantMeta': boolean;
  abstract readonly 'state': PaletteStateInterface;
}
...
const extractCandidatesScenarios: readonly ScenarioInterface<ExtractCandidatesInput, ExtractCandidatesOutput>[] = [
  {
    'assert': function(output, error) {
      ...
      const algorithms = output!.candidates.map((c) => { const result = c.algorithm; return result; }).sort();
      assert.deepStrictEqual(
        algorithms,
      ...
      for (const c of output!.candidates) {
        assert.ok(c.colorCount <= 4, `[cell=2b, scenario=default-candidates] candidate "${c.label}" respects shared k=4`);
      }
    },
    'input': {
      'input': {
  ...
  },
  {
    'assert': function(output, error) {
      ...
      const labels = output!.candidates!.map((c) => { const result = c.label; return result; }).sort();
      assert.deepStrictEqual(labels, ['compact', 'wide'], '[cell=2b, scenario=custom-candidates] labels match request');
      const compact = output!.candidates!.find((c) => {return c.label === 'compact';});
      const wide    = output!.candidates!.find((c) => {return c.label === 'wide';});
      assert.ok(compact!.colorCount <= 2, '[cell=2b, scenario=custom-candidates] compact candidate respects k=2');
      assert.ok(wide!.colorCount <= 4, '[cell=2b, scenario=custom-candidates] wide candidate respects k=4');
    },
    'input': {
      'input': {
        'bypass':    undefined,
  ...
  },
  {
    'assert': function(output, error) {
      ...
      assert.strictEqual(output!.hasDominantMeta, true, '[cell=2b, scenario=order-independence] gallery:extract dominantColors still written');
...
await new ScenarioRunner<ExtractCandidatesInput, ExtractCandidatesOutput>(
  ...
  (input) => {
    ...
    const state = engine.run(input.input);
    const candidates = ImageTestFixture.candidateSummaries(ImageTestFixture.metadata(state, 'gallery:candidates'));
    const dominantMetadata = ImageTestFixture.metadata(state, 'gallery:dominantColors');
    return {
...
abstract class AssignRolesOutput {
  abstract readonly 'hasAccent': boolean;
  abstract readonly 'hasCanvas': boolean;
  abstract readonly 'hasFrame': boolean;
  abstract readonly 'hasMuted': boolean;
  abstract readonly 'hasText': boolean;
  abstract readonly 'state': PaletteStateInterface;
}
...
await new ScenarioRunner<AssignRolesInput, AssignRolesOutput>(
...
abstract class HarmonizeOutput {
  abstract readonly 'afterHex': string | undefined;
  abstract readonly 'beforeHex': string | undefined;
  abstract readonly 'deltaE': number | undefined;
  abstract readonly 'harmonized': boolean | undefined;
  abstract readonly 'hueShift': number | undefined;
  abstract readonly 'state': PaletteStateInterface;
}
...
const harmonizeScenarios: readonly ScenarioInterface<HarmonizeInput, HarmonizeOutput>[] = [
  {
    'assert': function(output, error) {
      ...
      const harmonizedValue = ImageTestFixture.metadata(output!.state, 'gallery:harmonized');
      const harmonized = typeof harmonizedValue === 'boolean' ? harmonizedValue : undefined;
      const details = ImageTestFixture.harmonizeDetails(ImageTestFixture.metadata(output!.state, 'gallery:harmonizeDetails'));
...
await new ScenarioRunner<HarmonizeInput, HarmonizeOutput>(
  ...
  (input) => {
    ...
    const harmonizedValue = ImageTestFixture.metadata(state, 'gallery:harmonized');
    const harmonized = typeof harmonizedValue === 'boolean' ? harmonizedValue : undefined;
    const details = ImageTestFixture.harmonizeDetails(ImageTestFixture.metadata(state, 'gallery:harmonizeDetails'));
    return {
...
abstract class SchemaShapeInput {
  abstract readonly 'label': string;
}
abstract class SchemaShapeOutput {
  abstract readonly 'ok': boolean;
}
...
const schemaShapeScenarios: readonly ScenarioInterface<SchemaShapeInput, SchemaShapeOutput>[] = [
  ...
  },
  {
    'assert': function(_output, error) {
      ...
      const names = galleryRoleSchema5.roles.map((r) => { const result = r.name; return result; }).sort();
      assert.deepStrictEqual(
        names,
  ...
  },
  {
    'assert': function(_output, error) {
      ...
      const textRole = galleryRoleSchema5.roles.find((r) => {return r.name === 'text';});
...
await new ScenarioRunner<SchemaShapeInput, SchemaShapeOutput>(
  'ImagePlugin :: cell-5 :: galleryRoleSchema5',
  (_input) => { const ok = galleryRoleSchema5.roles.length === 5; return { 'ok': ok }; }
...
await test('ImagePlugin :: golden :: assignRoles full pipeline round-trip', () => {
  const engine = ImageTestFixture.engine();
  engine.pipeline(['intake:hex', 'gallery:extract', 'gallery:assignRoles']);
  const state = engine.run({
    'bypass': undefined,
