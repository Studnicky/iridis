...
await test('linear: t=0/0.5/1 passes through unchanged', () => {
  assert.strictEqual(linear(0), 0);
  assert.strictEqual(linear(0.5), 0.5);
  assert.strictEqual(linear(1), 1);
});

await test('cubicBezier: pinned endpoints, ease-like midpoint bounded in [0, 1]', () => {
...
await test('cubicBezier: linear control points approximate the identity curve', () => {
...
await test('spring: pinned endpoints, settles toward 1 near t=1', () => {
  const ease = spring();
...
await test('spring: custom stiffness/damping still pins endpoints', () => {
  const ease = spring({ 'damping': 10, 'mass': 2, 'stiffness': 80 });
  assert.strictEqual(ease(0), 0);
  assert.strictEqual(ease(1), 1);
});

await test('chromaticDetourHue: warm-to-cool sweep crossing the dead zone routes through green at t=0.5', () => {
...
await test('chromaticDetourHue: direct path outside the dead zone is unaffected', () => {
...
await test('chromaticDetourHue: endpoints match the source and target hues', () => {
