...
const paletteA: PaletteInterfaceType = {
  'background': { 'c': 0.05, 'h': 200, 'l': 0.2 },
  'text':       { 'c': 0.02, 'h': 100, 'l': 0.9 }
};

const paletteB: PaletteInterfaceType = {
  'background': { 'c': 0.15, 'h': 300, 'l': 0.8 },
  'text':       { 'c': 0.1, 'h': 20,  'l': 0.3 }
};

void test('lerp at t=0 returns a', () => {
  assert.deepStrictEqual(lerp(paletteA, paletteB, 0), paletteA);
});

void test('lerp at t=1 returns b', () => {
...
void test('lerp at t=0.5 returns the midpoint', () => {
...
void test('lerp hue wraparound: shortestArc from 350 to 10 passes through 360/0', () => {
  const a: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 350, 'l': 0.5 } };
  const b: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 10, 'l': 0.5 } };
  const mid = lerp(a, b, 0.5);
  assert.strictEqual(mid.role?.h, 0);
});

void test('lerp hue wraparound: clockwise from 350 to 10 also takes the short forward sweep here', () => {
  const a: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 350, 'l': 0.5 } };
  const b: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 10, 'l': 0.5 } };
...
void test('lerp hue wraparound: counterClockwise from 350 to 10 goes the long way through 180', () => {
  const a: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 350, 'l': 0.5 } };
  const b: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 10, 'l': 0.5 } };
...
void test('subtract wraps the hue delta to [-180, 180]', () => {
  const a: PaletteInterfaceType = { 'role': { 'c': 0.2, 'h': 10, 'l': 0.6 } };
  const b: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 350, 'l': 0.4 } };
...
void test('nearest picks the closest of several corpus entries', () => {
  const target: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 100, 'l': 0.5 } };
  const close: PaletteInterfaceType  = { 'role': { 'c': 0.1, 'h': 101, 'l': 0.51 } };
  const mid: PaletteInterfaceType    = { 'role': { 'c': 0.2, 'h': 150, 'l': 0.7 } };
  const far: PaletteInterfaceType    = { 'role': { 'c': 0.4, 'h': 300, 'l': 0.1 } };

  assert.strictEqual(nearest(target, [far, mid, close]), close);
});

void test('drift returns false below the threshold and true above it', () => {
  const current: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 100, 'l': 0.5 } };
  const derived: PaletteInterfaceType = { 'role': { 'c': 0.1, 'h': 100, 'l': 0.51 } };

  assert.strictEqual(drift(current, derived, 0.5), false);
  assert.strictEqual(drift(current, derived, 0.005), true);
});

void test('perpendicular only moves chroma on the targeted role', () => {
  const palette: PaletteInterfaceType = {
    'accent':     { 'c': 0.2, 'h': 250, 'l': 0.5 },
    'background': { 'c': 0.05, 'h': 200, 'l': 0.2 }
  };
...
void test('perpendicular clamps chroma into [0, 0.5]', () => {
  const palette: PaletteInterfaceType = { 'accent': { 'c': 0.48, 'h': 250, 'l': 0.5 } };
