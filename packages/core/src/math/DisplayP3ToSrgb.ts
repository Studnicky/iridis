...
export class DisplayP3ToSrgb {
  ...
  apply(r: number, g: number, b: number): RgbInterface {
    ...
    const sr = srgbEncode(clamp01.apply( 1.2249401 * rl - 0.2249404 * gl + 0.0000000 * bl));
    const sg = srgbEncode(clamp01.apply(-0.0420569 * rl + 1.0420571 * gl - 0.0000001 * bl));
    const sb = srgbEncode(clamp01.apply(-0.0196376 * rl - 0.0786361 * gl + 1.0982735 * bl));
