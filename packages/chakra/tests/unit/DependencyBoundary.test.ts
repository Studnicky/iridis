...
const CHAKRA_DIRECTORY = join(ROOT_DIRECTORY, 'packages/chakra');
...
class ChakraTestDependencyBoundary {
  private static sourceFiles(directory: string): string[] {
    const files: string[] = [];
    for (const entry of readdirSync(directory, { 'withFileTypes': true })) {
      const path = join(directory, entry.name);
      if (entry.isDirectory()) {
        files.push(...ChakraTestDependencyBoundary.sourceFiles(path));
  ...
  public static undeclaredImports(): string[] {
    ...
    const packageManifest = PackageManifest.parse(join(CHAKRA_DIRECTORY, 'package.json'));
    ...
    const testDirectory = join(CHAKRA_DIRECTORY, 'tests');
    for (const path of ChakraTestDependencyBoundary.sourceFiles(testDirectory)) {
      const imports = ts.preProcessFile(readFileSync(path, 'utf8'), true, true).importedFiles;
      for (const imported of imports) {
        const packageName = ChakraTestDependencyBoundary.packageName(imported.fileName);
        if (packageName !== undefined && !declared.has(packageName)) {
          violations.add(`${relative(CHAKRA_DIRECTORY, path)} imports ${imported.fileName}`);
        }
      }
    }
...
await test('chakra tests import only declared package dependencies', () => {
  assert.deepEqual(ChakraTestDependencyBoundary.undeclaredImports(), []);
});
